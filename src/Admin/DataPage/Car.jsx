...
const AdminPanel = () => {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const response = await axios.get('http://localhost:3000/cars'); // Updated URL
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCarId) {
        // Edit existing car
        const response = await axios.put(`http://localhost:3000/cars/${editingCarId}`, carData); // Updated URL
        setCars(cars.map(c => (c._id === editingCarId ? response.data : c)));
        setEditingCarId(null); // Clear editing state
      } else {
        // Add new car
        const response = await axios.post('http://localhost:3000/cars', carData); // Updated URL
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3000/cars/${id}`); // Updated URL
          ...
          <tbody>
            {cars.map((car) => (
              <tr key={car._id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">
                  <img src={car.imgUrl} alt={car.name} className="w-24 h-16 object-cover rounded-md" />
                </td>
                <td className="p-3">
                  <button
                  ...
                  <button
                    ...
                    className="ml-2 bg-red-600 text-white py-1 px-3 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500"
                  >
                    Delete
                  </button>
                </td>
